...
* Confirm whether folder creation for saved replies is available to all agents and team leads, or only to users with saved reply management permission.
* Confirm the exact activity types included in the customer profile activity timeline.
* Avoid implying that the activity timeline shows a complete customer history. The source only supports “recent” activity.
* Do not include specific ticket search performance figures unless verified figures are provided.
* Confirm whether the first response time filter uses business hours, calendar hours, or existing workspace reporting settings.
* Confirm where the first response time filter appears in the reporting interface before updating help documentation.
* Confirm whether the **Manage automations** permission is enabled by default for any existing custom roles.
* Avoid stronger security language for the automation permission unless reviewed and approved by product.
* Confirm that the SLA timezone update is a display-only change and that SLA calculation logic has not changed.
* Confirm whether users receive a notification when large CSV exports are ready.
* Do not promise a fix date for the CSV export known issue.
* Screenshots are likely needed for saved reply folders, the redesigned ticket details panel, and the role editor for automation permissions.
